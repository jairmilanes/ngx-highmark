...
import {Component, EventEmitter, SecurityContext} from '@angular/core';
import { TestBed } from '@angular/core/testing';

import { MarkdownModule } from './markdown.module';
import { errorSrcWithoutHttpClient, SECURITY_CONTEXT } from './markdown.service';
import { MarkedOptions } from './types';
...
@Component({
  ...
  template: `
    <div *ngIf="src; else dataTemplate">
      <markdown [src]="src" (error)="error.emit($event)"></markdown>
    </div>
...
describe('MarkdownModule', () => {
  ...
  describe('without HttpClient', () => {
    ...
    it('should throw an error when using src attribute', () => {
      ...
      expect(fixture.componentInstance.error.emit).toHaveBeenCalledWith(new Error(errorSrcWithoutHttpClient));
    });
  });
});
